...
class FakeCursor:
    ...
    def execute(self, query: str, params=None) -> None:
        ...
        if query.startswith("UPDATE jobs SET status = CASE WHEN retry_count + 1 >= 3 THEN 'failed' ELSE 'approval' END"):
            for job in self.connection.jobs:
                if job["status"] == "approval" and job.get("updated_minutes_ago", 0) > 30:
                    next_retry = job["retry_count"] + 1
                    job["retry_count"] = next_retry
                    job["error_log"] = "Renderer timed out after 30 minutes"
                    job["status"] = "failed" if next_retry >= 3 else "approval"
            return

        if query.startswith("SELECT job_id, source_path, clip_manifest, retry_count, render_path FROM jobs"):
            rows = [job for job in self.connection.jobs if job["status"] == "approval" and job["clip_manifest"] is not None]
...
class RendererServiceTests(unittest.TestCase):
    ...
    def test_run_once_recovers_stuck_approval_job(self) -> None:
        jobs = [
            {
                "job_id": "job-1",
                "source_path": "s3://bucket/sources/job-1/video.mp4",
